...
router.post('/', upload.single('image'), uploadLogger, createJobPosting);
router.get('/', getAllJobPostings);
router.get('/:id', validateParams(z.object({ id: z.string().uuid() })), getJobPosting);
router.put('/:id', upload.single('image'), uploadLogger, validateParams(z.object({ id: z.string().uuid() })), updateJobPosting);
